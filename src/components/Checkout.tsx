...
import { PembayaranTiket } from '../types/PembayaranTiket'; // Adjust path
...
function Checkout() {
    ...
    } = useQuery<CheckoutType, Error>({ // Use the renamed type
    ...
    if (!token) {
        return <div className="checkout-container status-message error">Please log in to view checkout details.</div>;
    }

    if (!paymentIdParam || isNaN(paymentIdParam)) {
         return <div className="checkout-container status-message error">Invalid Payment ID specified in the URL.</div>;
    }

    if (isLoading) {
        return <div className="checkout-container status-message loading">Loading Checkout Details...</div>;
    }

    if (error) {
        return <div className="checkout-container status-message error">Error loading checkout details: {errorMessage}</div>;
    }

    if (!checkoutData) {
        // This case might be covered by the 404 error handling, but good as a fallback
        return <div className="checkout-container status-message error">Checkout details not found for Payment ID: {paymentIdParam}.</div>;
    }
    ...
    return (
        <div className="checkout-container">
            ...
            {isFetching && <div className="status-message loading background">Checking for updates...</div>}
            ...
            <div className="checkout-summary">
                ...
                <div className="detail-item">
                    ...
                    <span className="detail-value">{formatDate(checkoutData.tanggal)}</span>
                </div>
                <div className="detail-item total-price">
                    <span className="detail-label">Total Amount Paid:</span>
                    <span className="detail-value">{formatCurrency(checkoutData.total_harga)}</span>
                </div>
                 <div className="detail-item">
                    <span className="detail-label">Record Created:</span>
                    <span className="detail-value">{formatDate(checkoutData.created_at)}</span>
                </div>
            </div>
            ...
            {pembayaran && (
                <div className="payment-details">
                    ...
                    <div className="detail-item">
                        ...
                        <span className="detail-value">{pembayaran.metode_pembayaran || 'N/A'}</span>
                    </div>
                    <div className="detail-item">
                        <span className="detail-label">Payment Status:</span>
                        <span className={`detail-value status-${pembayaran.status_pembayaran?.toLowerCase()}`}>
                            {pembayaran.status_pembayaran || 'N/A'}
                        </span>
                    </div>
                    <div className="detail-item">
                        <span className="detail-label">Number of Tickets:</span>
                        <span className="detail-value">{pembayaran.jumlah_tiket ?? 'N/A'}</span>
                    </div>
                    {/* Optionally display ticket info if available in pembayaran object */}
                    {/* {pembayaran.tiket && (
                        <div className="detail-item">
                            <span className="detail-label">Event:</span>
                            <span className="detail-value">{pembayaran.tiket.nama}</span>
                        </div>
                    )} */}
                     <div className="detail-item">
                        <span className="detail-label">Payment Recorded:</span>
                        <span className="detail-value">{formatDate(pembayaran.created_at)}</span>
                    </div>
                </div>
            )}
